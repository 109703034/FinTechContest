...
1. **初始化**:  
    使用 `Sentence-BERT` 模型生成所有文檔的語義嵌入，並保存到 FAISS 索引中。
...
3. **檢索**:
   使用 FAISS 索引進行檢索，將問題嵌入向量作為查詢，檢索索引中的最相似文檔段落，得到多個候選段落。
   
5. **計算分數**:
   對每個候選文檔計算總分數，具體包括以下三個子分數及其權重：
    | 項目           | 描述                                         | 權重 |
    |----------------|----------------------------------------------|------|
...
根據計算出的分數，得到最佳答案，輸出為 `faiss.json`。
